...
**Only the block is generated — pair it with two plain Gridfinity blanks.** The handle-rest zones at
each end aren't printed by this design; use ordinary
[`gridfinity_build123d`](https://github.com/Ruudjhuu/gridfinity_build123d) blanks (or any blank
Gridfinity bin) at 18 mm tall so the knives rest level. Sized by default for seven similarly-lengthed
kitchen knives (handles up to 26 mm wide, 2–3 mm spines) on a 3×2 Gridfinity footprint
(125.5 × 83.5 mm) — it prints flat in one piece, no splitting needed. If your knives differ, the
generator's `--knife-count`, `--handle-width-mm`, and `--handle-gap-mm` flags retune the whole layout.

**Got a knife with an unusually heavy handle?** One in my own set (a metal-handled carving knife)
rocked against a flat blank instead of resting still. Fixed it with a plain riser instead: the same
generator, no new geometry — `--grid-x 1 --grid-y 2 --height-mm 40 --no-cutouts` builds a small
raised block that supports the handle properly. Customising the tool for your own drawer is very
much the point.
...
(STL + 3MF attached). Pairs with two plain `gridfinity_build123d` blanks for the handle rests — only
the block itself is generated. Default sized for seven similarly-lengthed knives (2–3 mm spines,
