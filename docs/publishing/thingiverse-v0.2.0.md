...
**The block holds the blades; plain bins hold the handles.** The handle-rest zones at each end are
just ordinary open-top bins at 18 mm tall — the handles bridge across the rim, so the knives sit
level. Generate them with the same tool, no separate software needed:

```text
uv run python main.py --grid-x 3 --grid-y 4 --height-mm 18 --no-cutouts
```

(Any blank Gridfinity bin of that height works just as well if you already have some printed.)

The block itself is sized by default for seven similarly-lengthed kitchen knives (handles up to
26 mm wide, 2–3 mm spines) on a 3×2 Gridfinity footprint (125.5 × 83.5 mm) — it prints flat in one
piece, no splitting needed. If your knives differ, the generator's `--knife-count`,
`--handle-width-mm`, and `--handle-gap-mm` flags retune the whole layout.
...
(STL + 3MF attached). Pairs with two plain open-top bins for the handle rests, generated by the same
CLI (`--grid-x 3 --grid-y 4 --height-mm 18 --no-cutouts`). Default sized for seven
similarly-lengthed knives (2–3 mm spines, handles up to 26 mm); override with `--knife-count`,
`--handle-width-mm`, `--handle-gap-mm` for a different set.
