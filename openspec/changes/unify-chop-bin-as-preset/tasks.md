...
- [ ] 1.1 Create `cutlery_bin.py` with a `BinParameters` dataclass: grid_x, grid_y, height (units or mm), base
...
- [ ] 1.2 Add the `CutleryBin` divider fields to the parameter model: a `divisions` count (default 1) and a
  divider thickness.
- [ ] 1.3 Implement `validate()` accumulating all errors: grid range, height, pocket-fits-footprint, base/pocket
  corner radius, the cutout-fit checks gated by `cutouts_enabled`, and `divisions >= 1` / positive divider
  thickness.

## 2. KitchenBin geometry

- [ ] 2.1 Implement `KitchenBin` with a template-method build: Gridfinity base → open-top walls → explicit rounded
  pocket → divider hook (no-op here) → cutouts. Expose the pre-cutout divider hook for subclasses.
- [ ] 2.2 Port `ChopProfile` into `cutlery_bin.py` as `SideCutoutProfile`, and the post-#16 YZ-plane through-cut,
  gated by `cutouts_enabled`, cutting both X-perpendicular walls from the inner floor to the top.
- [ ] 2.3 Add a `create_kitchen_bin(params)` factory.

## 3. CutleryBin geometry

- [ ] 3.1 Implement `CutleryBin(KitchenBin)` that supplies straight, evenly-spaced dividers via the hook: parallel
  to the cut walls, spanning between the two un-cut walls, splitting the pocket into `divisions` equal columns.
- [ ] 3.2 Confirm the cutout runs through the dividers (the hook adds them before the cutout step).
- [ ] 3.3 Add a `create_cutlery_bin(params)` factory.

## 4. Presets

- [ ] 4.1 Add a presets registry mapping names to `BinParameters` factories; implement `chop-board` (a `KitchenBin`:
  pocket 220×160 mm, 35 mm radius, cutouts enabled, chop grid and height).
- [ ] 4.2 Implement preset resolution and listing: resolve by name, list available names, raise an actionable error
  for an unknown name.
...
- [ ] 7.2 Test the `KitchenBin` pocket builds valid geometry and yields non-uniform end/side walls (covers
  "Explicitly-sized pocket interior").
- [ ] 7.3 Test `CutleryBin` dividers: straight, evenly spaced, attached to both un-cut walls; `divisions=1` matches
  a `KitchenBin`; `divisions>=2` adds the expected columns (covers "CutleryBin dividers").
- [ ] 7.4 Test that with cutouts enabled and two or more columns, the slot runs through every divider and each stays
  attached at both ends (covers "Cutout passes through dividers").
- [ ] 7.5 Test `cutouts_enabled` default-on, `False` → solid walls/dividers, and fit validation only fires when
  enabled.
- [ ] 7.6 Test the `chop-board` preset reproduces the prior chop bin within tolerance (volume/bbox).
...
- [ ] 7.8 Test validation rejections: grid range, pocket-too-large, invalid divisions.
