...
- [x] 1.1 Change `check_print_bed` in `cutlery_bin.py` to
...
- [x] 1.2 In `main.py`, add `DEFAULT_BED_X_MM = 220`, `DEFAULT_BED_Y_MM = 220`, `DEFAULT_BED_Z_MM = 240` constants
...
- [x] 2.1 Restore/extend `check_print_bed` unit tests: fits within the volume; exceeds X; exceeds Y; exceeds Z; the
  warning names the dimension and the limit.
- [x] 2.2 Update the existing CLI test fakes to return a stub part exposing a `bounding_box()` (the check now runs
  on every invocation, so `main` always measures it).
- [x] 2.3 Add CLI tests: a normally-sized bin against the default volume produces no warning; an oversized model
...
- [x] 3.3 Add a `UAT.md` row exercising the bed-fit warning (a model with a deliberately small `--bed-x`/`--bed-y`/
  `--bed-z` prints a warning to stderr, names the axis, and still exports with exit 0).
- [x] 3.4 Generate the UAT models (the `UAT.md` bin cases) to `build/` for slicer review, confirming the geometry is
